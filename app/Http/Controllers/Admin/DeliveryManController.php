...
class DeliveryManController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $deliverymen->getCollection()->transform(function ($deliveryman) {
            ...
            // ✅ جلب جميع المعاملات المالية (transactions) لهذا الدليفري مان
            $transactions = DB::table('order_delivery_histories as odh')
                ->join('orders as o', 'odh.order_id', '=', 'o.id')
                ->join('delivery_men as dm', 'odh.delivery_man_id', '=', 'dm.id')
                ->select(
                    'odh.id as history_id',
                    'odh.delivery_man_id',
                    DB::raw("CONCAT(dm.f_name, ' ', dm.l_name) as delivery_man_name"),
                    'o.id as order_id',
                    'o.order_amount',
                    'o.delivery_charge',
                    DB::raw('(o.order_amount + o.delivery_charge) as total_amount'),
                    'o.payment_method',
                    'o.order_status',
                    'o.created_at as order_date'
                )
                ->where('odh.delivery_man_id', $deliveryman->id)
                ->where('o.payment_method', 'cash') // ✅ فقط المعاملات بالجنيه
                ->orderByDesc('o.created_at')
                ->get();
    
            // ✅ إضافة المعاملات إلى الكائن
            $deliveryman->transactions = $transactions;
    
            return $deliveryman;
        });
    
        return view('admin-views.delivery-man.list', compact(
            'deliverymen', 'search', 'status', 
            'activeCount', 'inactiveCount', 'startDateTime', 'endDateTime'
        ));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $deliveryman->identity_number = $request->identity_number??1;
