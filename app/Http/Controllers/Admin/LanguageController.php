...
class LanguageController extends Controller
{
     ...
     */
        public function lang($local): RedirectResponse
        {
            ...
            // Ensure $language is an array by decoding it if it's a JSON string
            if (is_string($language)) {
                $language = json_decode($language, true);
            }

            // Check if $language is iterable; if not, handle the error gracefully
            if (!is_array($language)) {
                // Log an error or set a default value if needed
                \Log::error("Language settings could not be decoded properly in lang method.");
                $language = []; // Default to empty array to avoid breaking the foreach
            }
